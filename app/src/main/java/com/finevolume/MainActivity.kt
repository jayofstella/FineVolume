...
override fun onCreate(b:Bundle?){super.onCreate(b);val sc=ScrollView(this);val root=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(36,54,36,54)};root.addView(TextView(this).apply{text="FineVolume v0.10.0";textSize=28f});root.addView(TextView(this).apply{text="真实/应用媒体音量接口验证版";textSize=16f});status=TextView(this).apply{textSize=15f;setPadding(0,20,0,15)};root.addView(status);button=Button(this).apply{text="一键执行 v0.10.0 验证并生成报告";setOnClickListener{start()}};root.addView(button);detail=TextView(this).apply{textSize=13f;setTextIsSelectable(true);setPadding(0,20,0,20);text="本版针对 v0.9.0 发现的 getStreamRealVolume / getAppMediaVolume 做精确签名与数值域验证。为避免盲写厂商接口，本次仍保持系统媒体音量档位不变。"};root.addView(detail);root.addView(TextView(this).apply{text="保持 Shizuku 运行、连接蓝牙耳机即可。测试后发送 下载/FineVolume/ 中的 v0.10.0 报告。";textSize=14f});sc.addView(root);setContentView(sc);Shizuku.addBinderReceivedListenerSticky(recv);Shizuku.addBinderDeadListener(dead);refresh("v0.10.0 已启动")}
...
private fun run(){val b=remote?:return;Thread{val d=Parcel.obtain();val r=Parcel.obtain();val text=try{b.transact(AvrcpUserService.TX_RUN_ALL,d,r,0);r.readException();r.readString()?:"empty report"}catch(e:Throwable){"FAILED ${e.javaClass.name}: ${e.message}"}finally{d.recycle();r.recycle()};val saved=save(text);runOnUiThread{detail.text=text+"\n\n"+saved;button.isEnabled=true;refresh("v0.10.0 完成")}}.start()}
private fun save(t:String):String=try{val n="FineVolume-TestReport-v0.10.0-${System.currentTimeMillis()}.txt";val v=ContentValues().apply{put(MediaStore.Downloads.DISPLAY_NAME,n);put(MediaStore.Downloads.MIME_TYPE,"text/plain");put(MediaStore.Downloads.RELATIVE_PATH,Environment.DIRECTORY_DOWNLOADS+"/FineVolume")};val u=contentResolver.insert(MediaStore.Downloads.EXTERNAL_CONTENT_URI,v)?:return "保存失败";contentResolver.openOutputStream(u)?.use{it.write(t.toByteArray())}?:return "保存失败";"已保存：下载/FineVolume/$n"}catch(e:Throwable){"保存失败：${e.message}"}
