...
class AvrcpUserService(private val context: Context) : Binder() {
    ...
    private fun runProbe():String{stateMessage="v0.9.0 fine attenuation probe running";val sb=StringBuilder();fun section(name:String){sb.append("\n===== ").append(name).append(" =====\n")};sb.append("FineVolume-TestReport v0.9.0\nuid=${Process.myUid()} pid=${Process.myPid()} timestampMs=${System.currentTimeMillis()}\nPurpose: find a non-root controllable attenuation/gain layer below STREAM_MUSIC without changing audible volume during discovery.\n");section("PRECHECK");sb.append(runShell("id",2,2000)).append('\n');sb.append("musicIndex=${try{music()}catch(_:Throwable){-1}}\n");sb.append("audio=${getService("audio")?.interfaceDescriptor}\npolicy=${getService("media.audio_policy")?.interfaceDescriptor}\n");section("AUDIO BINDER METHODS");sb.append(methods()).append('\n');section("AVAILABLE COMMAND SURFACES");sb.append(commands()).append('\n');section("RELEVANT SERVICES");sb.append(services()).append('\n');section("AUDIO POLICY SNAPSHOT");sb.append(policy()).append('\n');section("AUDIO FLINGER SNAPSHOT");sb.append(flinger()).append('\n');section("SAFETY RESULT");sb.append("No undocumented raw Binder transaction was sent. No volume index was changed. This probe only inventories callable gain/attenuation surfaces before the next targeted control test.\n");stateMessage="v0.9.0 probe complete; volume unchanged";return sb.toString()}
